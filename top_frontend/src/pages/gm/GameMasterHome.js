...
const GameMasterHome = () => {
  ...
  useEffect(() => {
    axios
      .get(host + "/gm/check_token", headers)
      .then((response) => {
  ...
  useEffect(() => {
    axios
      .get(host + "/gm/get_my_games", headers)
      .then((response) => {
  ...
  const handleGameDelete = (event) => {
    ...
    axios
      .delete(host + "/gm/delete_game/" + gameCreated.game_name, headers)
      .then((res) => {
  ...
  useEffect(() => {
    ...
    ws.current = new WebSocket(
      `${ws_host}/booking/connect/?room_token=${localStorage.getItem(
        "gm_token"
      )}&team_name=${encodeURIComponent(gameCreated.game_name)}`
    );
    ws.current.onopen = () => {
  ...
  return (
    <div className="relative">
      ...
      <div className="absolute w-screen h-screen flex flex-col justify-center items-center">
        {openGameCreateForm && (
          <CreateGameModal
            ...
            setOpenGameCreateForm={setOpenGameCreateForm}
          />
        )}
        ...
        {openAwardForm && (
          <AwardModal
            ...
            setOpenAwardForm={setOpenAwardForm}
          />
        )}
        {gameCreated === null ? (
          <div
            className="bg-green-400 rounded text-4xl p-4 text-white cursor-pointer select-none"
            onClick={() => {
              setOpenGameCreateForm(true);
            }}
          >
            Create Game
          </div>
          ...
          <>
            <div className="bg-blue-400 text-black w-96 h-fit rounded-t text-5xl text-center capitalize p-4">
              {gameCreated.game_name}
            </div>
            <div className="bg-blue-200 text-black w-96 h-fit rounded-b text-5xl text-center capitalize p-4">
              <div
                className={`bg-gray-200 p-4 text-center text-xl rounded ${
                  !!teamBooked && "bg-green-100 border-black border-2"
                }`}
              >
                Booked Team:{teamBooked}
              </div>
              <div
                className={`text-xl rounded cursor-pointer py-4 mt-2  ${
                  !!teamBooked ? "bg-green-400" : "bg-gray-400 "
                }`}
                onClick={() => {
                  setOpenAwardForm(!!teamBooked);
                }}
              >
                Award Points
              </div>
              <div
                className={`bg-green-300 text-xl rounded cursor-pointer py-4 mt-2 ${
                  !!teamBooked ? "bg-red-400" : "bg-gray-400 "
                }`}
                onClick={() => {
                  handleKick();
                }}
              ...
              <div
                className="bg-red-400 text-xl rounded cursor-pointer py-2 mt-4"
      ...
      <img
        className="absolute h-screen w-screen brightness-50 -z-10"
        src="https://plus.unsplash.com/premium_photo-1713200810940-97c3b2fefefd?q=80&w=1170&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
      />
    </div>
  );
};
